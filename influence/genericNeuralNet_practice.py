...
class GenericNeuralNet(object):
    ...
    def loss(self, logits, labels):
        ...
        loss_no_reg = tf.reduce_mean(cross_entropy, name='xentropy_mean') # loss of the entire batch
        tf.add_to_collection('losses', loss_no_reg)

        total_loss = tf.add_n(tf.get_collection('losses'), name='total_loss') # loss of all training so far
    ...
    def get_test_grad_loss_no_reg_val(self, test_indices, batch_size=100, loss_type='normal_loss'):
        # get the test gradient loss wrt the parameters, averaged over every instances of
        # testing points
