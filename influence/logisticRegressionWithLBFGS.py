...
class LogisticRegressionWithLBFGS(GenericNeuralNet):
    def __init__(self, input_dim, weight_decay, max_lbfgs_iter, num_to_remove, **kwargs):
        ...
        C_minus = 1.0 / ((self.num_train_examples - self.num_to_remove) * self.weight_decay)
        self.sklearn_model_minus = linear_model.LogisticRegression(
            C=C_minus,
            tol=1e-8,
            fit_intercept=False,
            solver='lbfgs',
    ...
    def inference(self, input):
        """
        Inference is when you are applying the trained model onto new data.
        (aka testing)
        Get the softmax_linear layer weights with weight decay (used to penalise
        big weights) using a truncated normal distribution.
        Save the weights tensor into an attribute, returns calculated logits.
        """
        with tf.variable_scope('softmax_linear'):
            ...
            logits = tf.matmul(input, tf.reshape(weights,
    ...
    def predictions(self, logits):
        """Performs softmax on logits, returns predictions."""
        preds = tf.nn.softmax(logits, name='preds')
        return preds

    def set_params(self):
        """Populate W_placeholder with weights acquired from tf.get_variables"""
    ...
    def train(self,
        ...
        self.train_with_LBFGS(
            feed_dict=self.all_train_feed_dict,  # filled with all train examples
            save_checkpoints=save_checkpoints,
            verbose=verbose)

    def train_with_LBFGS(self, feed_dict, save_checkpoints=True, verbose=True):

        X_train = feed_dict[self.input_placeholder]  # defined in placeholder_inputs()
        ...
        if num_train_examples == self.num_train_examples:
            ...
            if verbose: print('Using model minus')
            model = self.sklearn_model_minus
